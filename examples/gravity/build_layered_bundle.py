...
# First isolate and fix horizontal motion: deeper stages get progressively
# wider camera-space quads, while Y and Z are left untouched. This is applied
# directly in the main mesh-construction loop (no post-process/helper path).
HORIZONTAL_GEOMETRY_SCALES = [
    1.00,
    1.06,
    1.13,
    1.22,
...
def main() -> None:
    ...
    #
    # For this revision we intentionally tune only horizontal response first:
    # deeper layers get progressively wider X geometry, while Y and Z remain
    # exactly as generated by the normal perspective quad builder. This makes
    # left/right tilt separate more strongly toward the bottom without yet
    # changing the vertical-motion experiment.
    if len(HORIZONTAL_GEOMETRY_SCALES) != len(PARALLAX_DEPTHS):
        raise RuntimeError("horizontal scale count must match parallax depth count")

    meshes = []
    for idx in reversed(range(len(PARALLAX_DEPTHS))):
        quad_vertices, quad_indices = build_full_frame_quad(
            PARALLAX_DEPTHS[idx],
            aspect,
            args.fov,
        ...
        x_scale = HORIZONTAL_GEOMETRY_SCALES[idx]
        quad_vertices = [
            Vertex(
                x=v.x * x_scale,
                y=v.y,
                ...
                u=v.u,
                v=v.v,
                texture_slice=v.texture_slice,
            )
            for v in quad_vertices
        ]

        meshes.append((quad_vertices, quad_indices))
    ...
    project["generator"].update(
        {
            ...
            "horizontalGeometryScalesOuterToInner": HORIZONTAL_GEOMETRY_SCALES,
    ...
    print(
        "horizontal geometry scales outer -> inner: "
        f"{HORIZONTAL_GEOMETRY_SCALES}"
    )
    print(
